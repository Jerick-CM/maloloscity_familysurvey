...
export default {
    ...
    setup(props) {
        ...
        let modal = reactive({ show: false, status: true, note: "Saved." });
        ...
        const saveBusiness = async () => {
            business.value.user_id = Auth_user.value.id;
            await updateBusiness(route().params.id).then(() => {
                togglemodal();
            });
        };
        ...
        watch(
            ...
            (currentValue, oldValue) => {
                business.value.line_of_business = currentValue.value;
            },
            { deep: true }
        );
        watch(
            ...
            (currentValue, oldValue) => {
                business.value.strategic_location = currentValue.value;
            },
            { deep: true }
        );
        watch(
            () => qtr_paid.value,
            (currentValue, oldValue) => {
                business.value.qtr_paid = currentValue.value;
            },
            { deep: true }
        );
        watch(
            () => terms.value,
            (currentValue, oldValue) => {
                business.value.terms = currentValue.value;
            },
            { deep: true }
        );
        watch(
            () => code.value,
            (currentValue, oldValue) => {
                business.value.code = currentValue.value;
            },
            { deep: true }
        );
        watch(
            () => status.value,
            (currentValue, oldValue) => {
                business.value.status = currentValue.value;
            },
            { deep: true }
        );
        /* event change */
        ...
        <div class="py-2">
            ...
            <div class="w-full mx-auto sm:px-6 lg:px-8">
                ...
                <div id="template" class="">
                    ...
                    <form class="" @submit.prevent="saveBusiness">
                        <div>
